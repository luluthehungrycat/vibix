...
unsafe fn dev_id(vnode: *mut Vnode) -> DevId {
    match (*vnode).data as u32 {
        0 => DevId::Null,
        1 => DevId::Zero,
        // TtyS0 (2) is handled by TTY_OPS — never reaches devfs dispatch.
        _ => DevId::Null,
    }
}
...
unsafe fn devfs_read(vnode: *mut Vnode, buf: *mut u8, len: usize, _offset: &mut u64) -> isize {
    match dev_id(vnode) {
        ...
        _ => 0, // TtyS0 handled by TTY_OPS
    }
}

unsafe fn devfs_write(vnode: *mut Vnode, _buf: *const u8, len: usize, _offset: &mut u64) -> isize {
    match dev_id(vnode) {
        DevId::Null | DevId::Zero => len as isize,
        _ => len as isize, // TtyS0 handled by TTY_OPS
    }
}

unsafe fn devfs_lseek(_vnode: *mut Vnode, offset: i64, whence: u32) -> i64 {
    match whence {
        0 => offset,
        1 => offset,
        _ => -EINVAL as i64,
...
unsafe fn devfs_ioctl(_vnode: *mut Vnode, _request: u64, _arg: u64) -> i32 {
    // TtyS0 ioctl moved to TTY_OPS; Null/Zero have no ioctl.
    -ENOTTY
}
...
pub unsafe fn devfs_init() -> &'static mut Vnode {
    ...
    ptr::write(&mut DEV_TTYS0_VNODE, Vnode {
        ops: core::ptr::addr_of!(TTY_OPS) as *const VnodeOps,
        mode: V_CHR | 0o666,
        ino: 3,
        size: 0,
        fs_type: FsType::DevFS,
